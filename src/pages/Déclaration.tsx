  ...
  return (
    <section className="border-2 bg-white shadow-md rounded-lg p-4">
      <article className="grid grid-cols-12 text-gray-700 font-bold col-span-2 items-center">
        ...
        <span className={`p-2 col-span-2 flex flex-col`}>Date de Naissance</span>
      ...
      {declaration.map((item: any, index: number) => (
        ...
        >
          <span className={`p-2 col-span-2 flex flex-col`}>{FORMAT_DATE(item.registered)}</span>
          ...
          <span className={`p-2 col-span-2 flex flex-col`}>{FORMAT_DATE(item.child.brithDate)}</span>
          <span className={`p-2`}>
            <span className={`p-2 col-span-1.5 flex flex-col`}>{item.company.name}</span>
          </span>
          <span className={`p-2 col-span-1.5 flex flex-col`}>
